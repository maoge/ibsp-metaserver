...
('5', '1', 'MQ_RABBIT',   'MQ', 'rabbitmq-server-generic-unix-3.7.4.tar.gz', '/home/mq1/ftp/', 1456105739394),
('6', '1', 'MQ_ERLANG',   'MQ', 'otp_R20.3.tar.gz',                          '/home/mq1/ftp/', 1456105739394),
...
CREATE TABLE `t_alarm` (
  `SERVICE_ID`  varchar(36) NOT NULL COMMENT 'SERVICE ID',
  `INSTANCE_ID` varchar(36) NOT NULL COMMENT 'INSTANCE ID',
  `ALARM_CODE`  varchar(12) NOT NULL COMMENT 'ALARM CODE',
  `ALARM_DESC`  varchar(255) DEFAULT NULL COMMENT 'ALARM DESCRIPTION',
  `REC_TIME`    bigint(14)  NOT NULL COMMENT '添加时间',
  PRIMARY KEY `ALARM_IDX0` (`SERVICE_ID`,`INSTANCE_ID`,`ALARM_CODE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
CREATE TABLE `t_alarm_message` (
  ...
  `ALARM_CODE`  varchar(12) NOT NULL COMMENT 'ALARM CODE',
  `ALARM_DESC`  varchar(80) DEFAULT NULL COMMENT 'ALARM DESCRIPTION',
  `REC_TIME`    bigint(14)  NOT NULL COMMENT '添加时间',
...
DROP TABLE IF EXISTS `t_mo_mq_collect`;
CREATE TABLE `t_mo_mq_collect` (
  `VBROKER_ID`  varchar(36) COMMENT 'VBROKER_ID',
  `QUEUE_ID`  varchar(36) COMMENT 'QUEUE_ID 和vbroker_id之间只能有一个有值',
  `PRODUCE_RATE`  INT NOT NULL COMMENT '生产速率',
  `PRODUCE_COUNTS` bigint NOT NULL COMMENT '生产总数',
  `CONSUMER_RATE`  INT NOT NULL COMMENT '消费速率',
  `CONSUMER_COUNTS` bigint NOT NULL COMMENT '消费总数',
  `REC_TIME`    bigint(14)  NOT NULL COMMENT '添加时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;

DROP TABLE IF EXISTS `t_mo_mq_queue_detail`;
CREATE TABLE `t_mo_mq_queue_detail` (
  `VBROKER_ID`  varchar(36) NOT NULL COMMENT 'VBROKER_ID',
  `QUEUE_ID`  varchar(36) NOT NULL COMMENT 'VBROKER_ID',
  `CONSUMER_ID`  varchar(42) NOT NULL COMMENT 'CONSUMER_ID',
  `PRODUCE_RATE`  INT NOT NULL COMMENT '生产速率',
  `PRODUCE_COUNTS` bigint NOT NULL COMMENT '生产总数',
  `CONSUMER_RATE`  INT NOT NULL COMMENT '消费速率',
  `CONSUMER_COUNTS` bigint NOT NULL COMMENT '消费总数',
...
CREATE TABLE `t_mo_cache_proxy_collect` (
  ...
  `ACCESS_REDIS_CONNS` INT NOT NULL COMMENT 'Redis连接数量',
  `ACCESS_REQUEST_EXCEPTS` INT NOT NULL COMMENT '消费总数',
  `ACCESS_REQUEST_TPS` INT NOT NULL COMMENT '请求TPS',
  `REC_TIME`    bigint(14)  NOT NULL COMMENT '添加时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
DROP TABLE IF EXISTS `t_mo_tidb_collect`;
CREATE TABLE `t_mo_tidb_collect` (
  `INST_ID`  varchar(36) COMMENT 'INST_ID',
...
DROP TABLE IF EXISTS `t_mo_pd_collect`;
CREATE TABLE `t_mo_pd_collect` (
  `INST_ID` varchar(36) NOT NULL COMMENT 'INST_ID',
  `STORAGE_CAPACITY` bigint(14) NOT NULL COMMENT '集群总可用数据库空间大小',
  `CURRENT_STORAGE_SIZE` bigint(14) NOT NULL COMMENT '集群目前已用数据库空间大小',
...
DROP TABLE IF EXISTS `t_mo_tikv_collect`;
CREATE TABLE `t_mo_tikv_collect` (
  `INST_ID`  varchar(36) COMMENT 'INST_ID',
  `LEADER_COUNT` INT NOT NULL COMMENT 'leader数量',
  `REGION_COUNT` bigint(14) NOT NULL COMMENT 'region数量',
  `SCHEEDULER_COMMAND_DURATION` DOUBLE COMMENT '99%命令执行的时间',
  `TIME` bigint(14) NOT NULL COMMENT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
CREATE INDEX IDX5_T_MO_CACHE_NODE_COLLECT ON t_mo_cache_node_collect (INST_ID, REC_TIME);

CREATE INDEX IDX5_T_MO_CACHE_PROXY_COLLECT ON t_mo_cache_proxy_collect (INST_ID, REC_TIME);

CREATE INDEX IDX5_T_MO_MQ_COLLECT ON t_mo_mq_collect (VBROKER_ID,QUEUE_ID,REC_TIME);

CREATE INDEX IDX5_T_MO_MQ_QUEUE_DETAIL ON t_mo_mq_queue_detail (VBROKER_ID,QUEUE_ID,REC_TIME);

CREATE INDEX IDX5_T_MO_PD_COLLECT ON t_mo_pd_collect (INST_ID, TIME);

CREATE INDEX IDX5_T_MO_TIDB_COLLECT ON t_mo_tidb_collect (INST_ID, TIME);

CREATE INDEX IDX5_T_MO_TIKV_COLLECT ON t_mo_tikv_collect (INST_ID, TIME);
